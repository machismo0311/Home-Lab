...
\newcommand{\LabHost}{quarkylab}                       % SSH target over the VPN
                                                       %   (Headscale MagicDNS name,
                                                       %    or the 100.64.0.x address)
\newcommand{\HeadscaleURL}{https://headscale.kylemason.org} % Headscale login server  <-- CONFIRM
\newcommand{\AdminEmail}{kyle@kylemason.org}           % where students send their public key
...
\lstdefinestyle{shell}{
  ...
  escapeinside={(*@}{@*)},
  morekeywords={ssh,ssh-keygen,scp,rsync,sbatch,squeue,sacct,scancel,scontrol,cat,type,conda,source,python,tailscale,ping,mkdir,ls,cd,nano}
}
\lstdefinestyle{py}{
  language=Python,
  basicstyle=\ttfamily\small,
...
\begin{enumerate}
  ...
  \item \textbf{Join the private network} that \LabName{} lives on (a secure VPN).
...
The output is a single line beginning with \code{ssh-ed25519}. Copy that entire line and email it to your administrator at \textbf{\AdminEmail}, telling them your name and (if known) your assigned \code{studentNN} username.
...
%==============================================================================
\section{Step 2 --- Join the private network (VPN)}
%==============================================================================
\LabName{} is not exposed to the open internet. To reach it from anywhere, you join a private, encrypted network using a free app called \textbf{Tailscale}, connected to our own coordination server (\textbf{Headscale}). This is a one-time install plus a login.

\subsection{Install the Tailscale app}
\osbadge{Windows}
Download and run the installer from \url{https://tailscale.com/download/windows}. Accept the defaults. A Tailscale icon appears in your system tray.

\osbadge{macOS}
Install from the Mac App Store (search \emph{Tailscale}) or \url{https://tailscale.com/download/mac}. A Tailscale icon appears in your menu bar.
...
\subsection{Connect to our network}
Instead of Tailscale's default sign-in, point the app at our coordination server. Your administrator will give you either a \textbf{sign-up link} or a one-time \textbf{authentication key}.

\osbadge{Linux (and macOS/Windows with the CLI installed)}
\begin{lstlisting}
sudo tailscale up --login-server (*@\texttt{\HeadscaleURL}@*)
\end{lstlisting}
...
\begin{lstlisting}
sudo tailscale up --login-server (*@\texttt{\HeadscaleURL}@*) --authkey <key-from-admin>
\end{lstlisting}

\osbadge{Windows / macOS (graphical app)}
Right-click the Tailscale icon and choose \emph{Preferences} (or \emph{Use custom coordination server} / \emph{Change server}). Enter:
\begin{lstlisting}
(*@\texttt{\HeadscaleURL}@*)
\end{lstlisting}
Then click \emph{Log in} and complete the prompts your administrator described.

\subsection{Confirm you are connected}
\begin{lstlisting}
tailscale status
\end{lstlisting}
You should see \LabName{} (or \code{\LabHost}) listed. As a final check, make sure you can reach it:
\begin{lstlisting}
ping (*@\texttt{\LabHost}@*)
\end{lstlisting}
Replies mean the network is up. (Press \code{Ctrl}+\code{C} to stop \code{ping}.)

\begin{tipbox}
Leave Tailscale running whenever you want to use \LabName. If a connection later ``times out,'' the first thing to check is that Tailscale is connected (\code{tailscale status}).
\end{tipbox}

%==============================================================================
\section{Step 3 --- Connect to \texorpdfstring{\LabName}{the cluster}}
%==============================================================================
With your key added (Step~1) and the VPN up (Step~2), connect with one command. Use \emph{your} assigned username in place of \code{studentNN}:
\begin{lstlisting}
...
\code{Connection timed out} & The VPN is not connected. Run \code{tailscale status}; reconnect (Step~2). \\
\code{Could not resolve hostname} & Same as above --- the VPN provides the name \code{\LabHost}. \\
...
\begin{notebox}
Prefer a graphical editor? \textbf{Visual Studio Code} with the \emph{Remote --- SSH} extension lets you edit files on \LabName{} as if they were local, using the same \code{studentNN@\LabHost} address. It is optional but popular.
\end{notebox}
...
\begin{notebox}
Need a package that is not listed? It cannot be installed at job time (no internet in jobs). Email \AdminEmail{} and ask for it to be added to the container image.
\end{notebox}
...
%==============================================================================
Stuck, or need a package added to the environment? Contact the cluster administrator at \textbf{\AdminEmail}. When you write, include your \code{studentNN} username, the command you ran, and any error message you saw --- it makes help much faster.
...
\code{tailscale up -{}-login-server \HeadscaleURL} & Join the private network \\
...
\begin{description}[leftmargin=!,labelwidth=3.2cm,itemsep=3pt]
  ...
  \item[VPN] Virtual Private Network --- a secure, private tunnel that lets your computer reach \LabName{} from anywhere.
