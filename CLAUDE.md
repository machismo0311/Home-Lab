...
- Pursuing CCNA and an MIS degree at Cleveland State University
...
**km-cluster** — 7-node Proxmox cluster on PVE 9.2.3 (upgraded 2026-06-22); Randy stayed on 9.1.1 (kernel/ZFS-only upgrade — see Runbook/netframe_update_2026-06-22_upgrades.md)
...
### R730 Compute Nodes
> Dell service tags are not published here — see the private ops vault / Vaultwarden.
...
| CPUs | 2x E5-2690 v3 (24 cores / 48 threads) — corrected 2026-07-11 (nproc=48; was mislabeled v4/28c) |
| RAM | 128GB ECC DDR4 (→192GB planned: +64GB on hand as of 2026-07-11) |
| Kernel | 7.0.12-1-pve |
...
| PBS fingerprint | stored in the private ops vault / Vaultwarden (not published) |
...
| Prometheus/Grafana/Loki | LXC 103, pve3 (.183) | Grafana http://192.168.10.183:3000 | Stack active ✅; 8 nodes scraped; Prom/Loki localhost-only (F-03). **Alerting live 2026-07-10:** Grafana→Discord (no Alertmanager); `discord-alerts`(infra)+`discord-ups`(UPS); rules: InstanceDown/PiholePrimaryDown/PiholeSecondaryDown/ZfsPoolDegraded/GpuTempHigh/GpuMemoryHigh/DiskAlmostFull/LowMemory/UPS. Added blackbox DNS probe (.177/.178) + ZFS pool textfile collector + nvidia-smi GPU collector (QuarkyLab/Jarvis). Config-as-code: private repo machismo0311/netframe-monitoring-stack. See Runbook/Monitoring-Alerting-2026-07-10.md |
...
| step-ca | pve2 | https://192.168.10.204:443 | *.netframe.local TLS — active ✅ CA password stored in Vaultwarden |
...
- **DS4246 JBOD:** 16x 4TB SAS via Randy's LSI 9207-8e (IT mode) + multipath. **Pool `bulk` BUILT & ONLINE 2026-07-08** — 2× 8-wide RAIDZ2, 58.2T raw / ~41.3 TiB usable, reboot-verified (auto-imports cleanly). See Runbook/DS4246-Pool-Buildout-Plan-2026-07-07.md
...
- **OPNsense DR (Tier A, 2026-07-10):** serial console verified — `qm terminal 100` from pve2 (exit Ctrl-O) — emergency access works. Nightly age-encrypted `config.xml` backup → private repo `machismo0311/opnsense-config-backup` (Ares cron 03:17, DR-tested decrypt). Cold-restore = `RESTORE.md` in that repo. age private key in Vaultwarden + Ares `~/.config/opnsense-backup/age-key.txt`. **OPNsense qemu-guest-agent installed + working 2026-07-11** (`agent:1` + `pkg install os-qemu-guest-agent` → `qemu-guest-agent-10.0.2` + service started/persisted; `qm agent 100 ping` rc=0). The plugin had to be pkg-installed from the console shell — the GUI falsely showed it installed. OPNsense SSH is disabled → console (`qm terminal 100`, menu opt 8) is the only shell; ACPI graceful shutdown works regardless. See Runbook/DNS-HA-OPNsense-Resilience-2026-07-10.md.
- **CI/CD (2026-07-10):** GitHub Actions lint/syntax on push+PR — Home-Lab (ruff+py_compile+shellcheck; **branch protection** requires checks), netframe-monitor (ruff-critical+py_compile+shellcheck), opnsense-config-backup (shellcheck), netframe-monitoring-stack (yamllint+shellcheck). Private repos can't use server-side protection on the free plan → **pre-push hooks** (`.githooks/pre-push` + `install-hooks.sh`, run per clone; bypass `git push --no-verify`; needs ruff/shellcheck/yamllint on PATH). **Deployment stays manual** (SSH/systemd/API — no auto-deploy to infra). See Runbook/CI-CD-2026-07-10.md.
- **RKE2 k8s (Phase 1, 2026-07-10):** 3-node HA control plane — `rke2-cp1/2/3` (VMs 201/202/203 on pve3/4/5, `.51/.52/.53`, VLAN 1), etcd quorum, Cilium CNI, **API VIP `192.168.10.54:6443`** (kube-vip), MetalLB pool `.71-.75`. kubectl from Ares `KUBECONFIG=~/.kube/config-rke2`. **CPU-only — GPU scheduling DEFERRED** (SLURM/Ollama own the cards; see scripts/slurm/README.md). **Phase 2 done:** CP nodes run workloads (schedulable, no dedicated workers — avoids GPU-server contention); Randy `datastore/k8s` NFS → default StorageClass `nfs-client` (nfs-subdir provisioner; nodes need `nfs-common`). Nothing existing touched. **Phase 5 (2026-07-11):** Randy joined as a **bare-metal RKE2 agent on the PVE storage host** (`node randy` Ready, `v1.35.6+rke2r1`) — kubelet reservations sized to measured hw (system-reserved 6CPU/24Gi + kube-reserved 2CPU/4Gi + eviction 2Gi → Allocatable **40 CPU / ~95.8Gi**; `enforce=pods` so ZFS/PBS aren't cgroup-throttled), tainted `node.netframe.io/role=storage:NoSchedule` (infra DaemonSets tolerate). Config `/etc/rancher/rke2/config.yaml` (0600). **⚠️ agents register on supervisor `:9345`, NOT apiserver `:6443`** (6443→`failed to get CA certs: Unauthorized`). Post-join verified: corosync 7/7, zpools ONLINE, NFS/PBS intact, VLAN30 egress unchanged. Rollback `rke2-agent-uninstall.sh`. See Runbook/RKE2-Phase1-HA-ControlPlane-2026-07-10.md §Phase 5.
...
- Wazuh VM 104 (QuarkyLab, .184): **qemu-guest-agent installed + working 2026-07-10** (`agent:1` set, channel live, `qm agent 104 ping` responds) → QuarkyLab host reboots now shut it down **gracefully** (no more unclean hard-stop / `wazuh-indexer` corruption / dashboard 503). The old manual workaround is only needed if it ever comes back unhealthy anyway: `qm stop 104 && qm start 104`, wait ~4 min; healthy = dashboard root 302→/app/login (NOT 200), manager `:55000`=401, indexer `:9200`=000/refused from LAN is normal. onboot=1 set.
...
- Headscale Phase 2 pending: QuarkyLab + the researcher's Mac must migrate together — do not migrate one without the other (device/user identifiers kept in the private ops vault)
