...
| IP | **192.168.10.153** via DHCP (static mapping still pending, see below) |
| DNS | `homeassistant.netframe.local` → .153 on primary Pi-hole (nebula-sync mirrors to .178) |
...
2. **DHCP static mapping:** OPNsense → Services → DHCPv4 → LAN → map MAC `BC:24:11:27:B2:5C` to `192.168.10.153` so the Pi-hole DNS record can never drift. (Not automated: our OPNsense API key is read-only by design.)
...
- **IoT VLAN 40:** HA sits on VLAN 1 (mgmt) today because that is where pve5's bridge is. IoT devices belong on VLAN 40; when the first ones arrive, add OPNsense rules allowing HA (.153) → VLAN 40 (and mDNS/SSDP reflection via the Avahi/udpbroadcastrelay approach, or use the HA integrations that take static IPs). Alternative later: trunk pve5's switch port and give VM 110 a second NIC tagged VLAN 40.
