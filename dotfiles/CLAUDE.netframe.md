...
- R730 GPU fan noise (QuarkyLab/Jarvis): the ~3,800 RPM idle fan floor is **inherent** to the dual-Xeon + GPU config (high-performance fans), NOT a misconfiguration. Ramp is by design (automatic, `ThermalProfile=Default`) and thermal headroom is huge, but the ramp was **NOT empirically triggered** — measured 2026-07-04, an 88-core CPU load (loadavg 67.8) raised exhaust only 33→34 °C (limit 70 °C), i.e. cooling absorbed it *without* needing to ramp. **Confirm the ramp actually fires under real GPU load during the Jarvis install** (watch `nvidia-smi` temp + `ipmitool sdr type fan` together; abort if GPU >85 °C and fans aren't climbing). Do NOT chase it: thermal offsets have no effect (`FanSpeedOffset`, `MinimumFanSpeed`, `ThirdPartyPCIFanResponse` all tested → zero change), and manual `ipmitool raw 0x30 0x30` static control disables auto-ramp = unsafe on a GPU node. GPU thermal config to keep/replicate = `ThirdPartyPCIFanResponse=Disabled` + `ThermalProfile=Default`; read/set via iDRAC-8 SCP export/import (racadm + Redfish DellAttributes both unavailable on fw 2.86) from Ares' `enp0s31f6.20`. Full investigation in Compute/Dell R730 - ML Node.md.
