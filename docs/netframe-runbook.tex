...
\usepackage{multirow}
\usepackage{tabularx}
\usepackage{amssymb}   % \square, \checkmark
...
\titleformat{\section}[block]
  ...
  {}{0pt}
  {\colorbox{sectionblue}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\thesection\quad #1}}}
\titleformat{\subsection}{\normalsize\bfseries\color{nfcyan}}{\thesubsection}{0.5em}{#1}
\titleformat{\subsubsection}{\normalsize\bfseries\color{nfyellow}}{\thesubsubsection}{0.5em}{#1}
...
\begin{center}
  ...
  {\normalsize
    \begin{tabular}{rl}
      ...
      \color{nfcyan}Revised:   & 2026-07-10 \\
  ...
  EC-135/145 Instructor Pilot $\cdot$ Check Airman \\
    Aviation Safety / FOQA Officer \\
    Network Engineering Candidate --- CCNA in progress
  }
\end{center}
\end{titlepage}
\nopagecolor
...
Proxmox VE 9.2.3 across seven nodes as of July 2026 (Randy stayed on 9.1.1 —
a kernel/ZFS-only upgrade).
...
OPNsense runs as VM 100 on pve2 (192.168.10.204). Version 25.1.12.
...
\begin{infobox}[Randy — Commissioned 2026-06-22]
\textbf{Role:} Primary storage node, Proxmox Backup Server \\
\textbf{IP:} 192.168.10.187 \quad \textbf{IPMI:} 192.168.20.22 (VLAN 20) \\
\textbf{OS:} Proxmox VE 9.1.1 (kernel 7.0.12-1-pve) \\
\textbf{CPU:} Dual Intel E5-2690 v3 (24 cores / 48 threads) \\
...
\begin{itemize}
  ...
  \item Data drives: JBOD mode enabled, all 22 data drives passed through to OS
...
\begin{itemize}
  ...
  \item Fingerprint: stored in Vaultwarden (not published)
...
\begin{itemize}
  \item \textbf{nic3} (Mellanox ConnectX-3): 10G UP $\rightarrow$ xe-0/2/0 on EX3400
  \item \textbf{Dual-homed (2026-07-02):} mgmt/corosync on VLAN 1 (192.168.10.187); NFS/PBS/egress on VLAN 30 (192.168.30.187)
  \item \textbf{nic0, nic1} (onboard Intel): unused
...
echo "deb http://download.proxmox.com/debian/pve trixie pve-no-subscription" \
  > /etc/apt/sources.list.d/pve-no-subscription.list   # trixie (Debian 13/PVE 9); NOT bookworm
...
zpool create -f -o ashift=12 datastore \
  ...
  raidz2 <4x Seagate ST2000NX0423>   # 4th vdev, added in the 2026-06-25 expansion
...
\textbf{Role:} the researcher's ML research node \\
\textbf{iDRAC:} 192.168.20.20 (VLAN 20; creds in Vaultwarden) \\
\textbf{CPU:} Dual E5-2699 v4 (matched stepping) \quad \textbf{RAM:} 512 GB LRDIMM ECC \\
\textbf{GPU:} RTX 8000 48 GB (installed 2026-07-01) \quad \textbf{BIOS:} 2.19.0
\end{infobox}

\textbf{GPU stack (verified 2026-07-01):} NVIDIA 550.163.01 on kernel 6.14.11-9-pve,
pinned via \texttt{GRUB\_DEFAULT} (6.17+ breaks the nvidia-550 DRM API). \texttt{nvidia-smi}
reports 48 GB (driver-free Turing swap from the earlier RTX 6000).

\begin{lstlisting}[language=bash, caption={Kernel pin for the NVIDIA 550 stack (QuarkyLab)}]
apt install pve-kernel-6.14.11-9-pve
# Pin as default in /etc/default/grub (GRUB_DEFAULT), then:
update-grub && reboot
# NVIDIA 550.163.01 driver already installed & verified -- do NOT upgrade the kernel
\end{lstlisting}
...
\textbf{iDRAC:} 192.168.20.21 (VLAN 20; creds in Vaultwarden) \\
\textbf{CPU:} Dual E5-2687W v4 \quad \textbf{RAM:} 384 GB LRDIMM ECC \\
\textbf{GPU:} 2x RTX 6000 — 24GB each / 48GB total (installed 2026-07-04)
\end{infobox}

Ollama serves Qwen2.5 72B Q4\_K\_M, GPU-backed across both cards (models on the
\texttt{tank/models} ZFS dataset). FastAPI \texttt{llm\_router.py} implements hybrid
routing — local Ollama by default, escalating to the Claude API on an explicit
\texttt{escalate} flag, a \texttt{model=claude-*} request, or local failure (Ollama
exposes no logprobs, so routing is by flag/model/failure, not confidence).
...
Headscale v0.29.1 running as LXC 105 on pve5. Phase 1 (2026-06-22) migrated
Ares (100.64.0.1), Randy (.2), pve5 (.3), pve4 (.4), pve3 (.5) and Jarvis (.6)
to self-hosted. Phase 2 (QuarkyLab + the researcher's Mac) is pending.
...
\begin{itemize}[leftmargin=*]
  \item[\color{nfgreen}$\checkmark$] VLAN activation — EX3400 trunk live, verified end-to-end (2026-06-25)
  \item[\color{nfgreen}$\checkmark$] QuarkyLab RTX 8000 + Jarvis 2x RTX 6000 installed \& GPU-backed (2026-07)
  \item[\color{nfgreen}$\checkmark$] Backup schedules to randy-pbs (daily, 7d+4w retention)
  \item[\color{nfgreen}$\checkmark$] DS4246 — pool \texttt{bulk} built \& online (2x 8-wide RAIDZ2, ~41.3 TiB, 2026-07-08)
  \item[\color{nfgreen}$\checkmark$] Jellyfin on Randy (v10.11.11); RX 580 ROCm transcoding pending power cable
  \item[\color{nfgreen}$\checkmark$] UPS monitoring live (NUT $\rightarrow$ PeaNUT $\rightarrow$ Homepage/Grafana)
  \item[\color{nfred}$\square$] Headscale Phase 2 — QuarkyLab + the researcher's Mac
  ...
  \item[\color{nfred}$\square$] RKE2 / Kubernetes (Cilium, MetalLB, NVIDIA GPU Operator)
\end{itemize}
